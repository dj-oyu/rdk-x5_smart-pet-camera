...
- **実装**: `_merge_detections_with_nms()` + 検出キャッシュ
...
1280x720 フレームに対して 640x640 の ROI を中央寄せ (y=40) で配置すると、
上下に参照されない領域 (デッドゾーン) が発生する。
...
合計: 80px (11.1%) が参照されない
```

### 解決策: パターン巡回方式 (採用)

3つの Y 位置パターンを巡回することで全域をカバー。
...
```
Frame 0: パターン0, ROI 0 (左上)
Frame 1: パターン0, ROI 1 (右上)
Frame 2: パターン1, ROI 0 (左中央)
Frame 3: パターン1, ROI 1 (右中央)
Frame 4: パターン2, ROI 0 (左下)
Frame 5: パターン2, ROI 1 (右下)
→ 6フレームで全域カバー、統合NMS出力
```

#### オーバーラップ領域

各パターン間で 600px のオーバーラップがあり、境界付近の物体も確実に検出。

```
パターン0: y=0-640
パターン1: y=40-680   (パターン0と 600px オーバーラップ)
パターン2: y=80-720   (パターン1と 600px オーバーラップ)
```

### 将来拡張: 適応的ROI選択

パターン巡回をベースに、検出結果に応じて動的にROI位置を調整。
...
class AdaptiveROISelector:
    def __init__(self):
        self.patterns = [
            [(0, 0), (640, 0)],      # パターン0: 上寄せ
            [(0, 40), (640, 40)],    # パターン1: 中央
            [(0, 80), (640, 80)],    # パターン2: 下寄せ
        ]
        self.pattern_index = 0
        self.last_detections = []

    def get_next_roi(self) -> tuple[int, int, int, int]:
        if self.last_detections:
            # 検出追跡モード: 検出位置周辺にフォーカス
            return self._roi_around_detection()
        else:
            # 探索モード: パターン巡回で全域スキャン
            return self._next_pattern_roi()
```

| モード | 全域カバー周期 | 用途 |
|--------|---------------|------|
| パターン巡回 | 6フレーム | 探索・初期検出 |
| 検出追跡 | 2フレーム | 追跡中の高速更新 |
| ハイブリッド | 可変 | バランス |
