...
## 6. 導入結果
| 指標 | 最適化前 | VSE最適化後 | モデル切替後 (2026-01) |
| :--- | :--- | :--- | :--- |
| **CPU負荷 (Detector)** | 高 | **極低** | 極低 |
| **推論遅延 (前処理)** | ~20ms | **< 1ms** | < 1ms |
| **BPU推論遅延** | - | ~46ms (yolov13n) | **~9ms (yolo11n)** |
| **夜間検出精度** | 不安定 | **向上 (ガンマ補正効果)** | 向上 |
| **映像品質** | フリッカーあり | **安定 (独立バッファ)** | 安定 |

> **2026-01-28 更新**: BPU プロファイリングにより yolov13n (45.5ms) → yolo11n (8.9ms) に切替。
> 起動スクリプトのデフォルト `--yolo-model v11n` で動作。詳細は `docs/yolo_bpu_profiling_report.md`。
