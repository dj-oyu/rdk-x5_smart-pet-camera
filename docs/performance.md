...
| SHM→Goヒープ転送 | `ReadLatestCopy()` + CGo `hb_mem_import` + memcpy | ~30-40% |
| NAL処理・RTPパケット化 | pion/webrtc v4 (Go) | ~20-30% |
| SRTP暗号化 | pion/srtp v3（ソフトウェアAES） | ~40-50% |
| DTLS/ICE/RTCP管理 | pion Go runtime | ~10% |

**エンコーダー設定** (`src/capture/encoder_lowlevel.c`):
- コーデック: H.265 CBR、ビットレート: 600 kbps（デフォルト）
- キーフレーム間隔: 30f（1秒）、QP範囲: 8-50

### streaming-server 120% の内訳推定（WebRTC 1クライアント接続中）

| 要因 | CPU推定 | 備考 |
|------|--------|------|
| SRTP暗号化（ソフトウェアAES） | ~40-50% | ARMv8 AES命令なし（`/proc/cpuinfo` `Features` に `aes` なし） |
| SHM→Goヒープ memcpy (30fps) | ~30-40% | 2ステージパイプラインのため意図的な設計（`ReadLatestCopy()`） |
| pion Goランタイム（goroutine、GC） | ~20-30% | |
| NAL処理・RTPパケット化 | ~10% | |

**SRTPについて**: このSoCはARMv8暗号拡張命令（`aes`/`sha2`/`pmull`）を搭載していないため、pion/srtp v3はソフトウェアAESで動作。WebRTC仕様上SRTPは必須であり、削減不可。

**memcpyについて**: `ReadLatest()`（ゼロコピー）→`ReadLatestCopy()`への変更は、SHM reader goroutine と WebRTC sender goroutine を非同期化するための意図的な設計。VPUバッファのライフタイムをGoのGCに委ねることでシリアライズを回避している。
