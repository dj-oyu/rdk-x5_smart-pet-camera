...
| シナリオ | CPU (pion, `ps`) | CPU (自前WebRTC, `ps`) | CPU (自前WebRTC, `top` 1core) |
|---------|-----------------|---------------------|------------------------------|
| streaming-server アクティブ（2クライアント） | ~52% | **~10%** | ~90% (1core占有) |
| streaming-server アクティブ（1クライアント） | ~40% | **~5%** (推定) | ~45% (推定) |
| streaming-server アイドル（クライアント0） | ~0.7% | **~1%** | ~1% |
| detector 夜間 idle（T2スロットリング） | ~25% | ~25% | — |
| detector 夜間 active（motion/YOLO） | ~180% | ~180% | — |
| detector 昼間（YOLO常時実行） | ~40% | ~40% | — |

※ `ps` は全コア平均の累積値、`top` は 1 core 上のリアルタイムピーク。SendFrame が 1 goroutine で直列処理するため、バースト送信中は 1 core をほぼ占有する。

**自前 WebRTC 実装（2026-04-16）**: pion/webrtc (17 modules) を排除し、SDP/ICE-lite/DTLS(pion/dtls のみ)/SRTP/RTP を自前実装。OP-TEE AF_ALG ハードウェア暗号も実装・検証したが、TE context-switch overhead により ソフトウェア暗号 (Go crypto/aes + crypto/hmac) の方が高速だったため、production ではソフトウェア暗号を使用。詳細は `docs/optee-afalg-findings.md`。
...
| SHM→Goヒープ転送 | `ReadLatestCopy()` + CGo `hb_mem_import` + memcpy | 小 (0.9%) |
| NAL処理・RTPパケット化 | 自前 `rtppack.PacketizeH265` (Go) | 小 |
| SRTP暗号化 | 自前 AES-128-CTR + HMAC-SHA1 (Go crypto) | **支配的** (AES 35% + SHA1 20%) |
| UDP送信 | `net.UDPConn.WriteToUDP` | 中 (18%) |
...
### streaming-server の内訳実測（2026-04-16、pprof 15秒、WebRTC 2クライアント）

`go tool pprof` による CPU プロファイル実測値（total samples = 12.28s）。

| 要因 | flat | 割合 | 備考 |
|------|------|------|------|
| SRTP AES暗号化 (`encryptBlockGeneric`) | 4.10s | **33.4%** | ソフトウェアAES-128-CTR（ARMv8暗号拡張なし） |
| SRTP HMAC-SHA1 (`sha1.blockGeneric`) | 2.64s | **21.5%** | SRTP認証タグ生成、パケットごとに実行 |
| UDP送信 syscall | 1.96s | **16.0%** | `Syscall6`（WriteTo per RTP packet） |
| NACK Interceptor バッファコピー | 0.24s | 2.0% | `rtpbuffer.PacketFactoryCopy`（単方向配信では不要） |
| CGo + SHM読み取り | 0.11s | 0.9% | `cgocall`（hb_mem_import + memcpy） |
| Goランタイム（GC、スケジューラ） | ~0.8s | ~6% | `findRunnable`, `netpoll`, `mallocgc` 等 |

**SRTP暗号化が支配的な理由**:
- 旧推定は「実効帯域195kbps → 暗号化量が極小」としていたが、**パケット数**を考慮していなかった
- H.265 30fps × MTU 1200byte分割 → 多数のRTPパケットが生成され、パケットごとにAES-CTR暗号化 + HMAC-SHA1認証タグ生成が実行される
- 2クライアント接続時は全パケットが2倍になる
- このSoCはARMv8暗号拡張命令（`aes`）を搭載しない → `encryptBlockGeneric`（ソフトウェア実装）にフォールバック
- OpenSSLの「~50 MB/sec」ベンチマークはバルク暗号化の測定値であり、パケットごとの鍵スケジュール・HMAC初期化のオーバーヘッドを含まない

**旧推定の誤り**: CGo + memcpy を「主要コスト」としていたが、実測では0.9%に過ぎない。`ReadLatestCopyBuf` + `sync.Pool` 最適化（PR #171）により十分に軽量化されていた。
...
## 7. Interceptor チェーン最適化（2026-04-16）

pion/webrtc v4 のデフォルト interceptor（NACK Generator/Responder, TWCC, Stats, Receiver/Sender Report）を、
単方向 H.265 配信に必要な **Sender Report のみ**に削減。

### 変更内容

`internal/webrtc/server.go` で明示的に `interceptor.Registry` を構成し、`webrtc.ConfigureRTCPReports` のみ登録。

除外した interceptor:
- **NACK Responder**: 毎パケット `PacketFactoryCopy` で RTP バッファをコピー。単方向配信で再送不要
- **NACK Generator**: 受信側で使用。送信専用サーバーでは不要
- **TWCC (Transport-Wide CC)**: 帯域推定。固定ビットレート H.265 では不要
- **Stats**: RTP ストリーム統計収集。Prometheus メトリクスで代替済み

### 効果（pprof 15秒、WebRTC 2クライアント）

| メトリクス | Before | After | 変化 |
|-----------|--------|-------|------|
| Total samples | 12.28s (81.9%) | 10.99s (73.3%) | **-10.5%** |
| `ps aux` CPU | 55.9% | 52.4% | **-3.5pt** |
| NACK `PacketFactoryCopy` | 0.24s (2.0%) | 0 | 除去 |
| Interceptor チェーン深度 | 5段 | 1段 (report のみ) | -4段 |

### 残存ボトルネック: SRTP ソフトウェア暗号化

SRTP 暗号化（AES-128-CTR + HMAC-SHA1）が pprof サンプルの ~55% を占める。
このSoCは ARMv8 暗号拡張命令を搭載しないため `encryptBlockGeneric`（pure Go テーブルルックアップ実装）が使われる。
SendFrame が 1 goroutine で直列処理するため、バースト送信中は 1 core をほぼ占有する (`top` ~90%)。

### OP-TEE ハードウェア暗号エンジン調査・実装・結果（2026-04-16）

RDK X5 は OP-TEE (ARM TrustZone) ベースのハードウェア暗号エンジンを搭載。
AF_ALG 経由でアクセスし、AES-CTR + HMAC-SHA1 を Go で実装・NIST テストベクターで検証した。

**実装と TE ドライバ bug**: 3 つの非標準挙動を発見しワークアラウンドを実装。詳細は `docs/optee-afalg-findings.md`。

**初期ベンチマーク（isolated、Python 経由）**: 7.7x 高速と見積もったが、**production 環境では逆に遅かった**:

| 構成 | CPU (`top` 1core) | CPU (`ps` 累積) |
|------|-------------------|-----------------|
| pion/srtp ソフトウェア | ~200%+ | ~52% |
| 自前 SRTP + AF_ALG (AES+HMAC) | ~180% | **~47%** |
| **自前 SRTP ソフトウェア** | **~90%** | **~10%** |

**原因**: AF_ALG の syscall (sendmsg/read) ごとに Normal World → Secure World (TrustZone) コンテキストスイッチが発生。1200B パケットに対して ~6 syscall/packet × 1500 pkt/sec = 9000 world-switch/sec。この overhead がソフトウェア AES の CPU コストを大幅に上回る。

**結論**: TE ドライバは大データ一括処理 (Widevine DRM 等) 向けに設計されており、per-packet 高頻度暗号化には不適。production はソフトウェア暗号を使用。AF_ALG 実装は参照用に `internal/srtp/afalg.go` に残存。
...
| 施策 | 予想効果 | 難易度 | 状態 |
|------|---------|-------|------|
| ~~SRTP を AF_ALG (OP-TEE) へオフロード~~ | ~~~52% → ~5%~~ 実測: 47% (悪化) | ~~高~~ | **不採用** — TE overhead > SW crypto |
| SRTP 暗号化を並列化 (session ごとに goroutine) | `top` ~90% → ~45% (2core分散) | 低 | 検討中 |
| ARMv8 暗号拡張命令付き SoC へ移行 | AES 35% → ~1% | — | ハードウェア依存 |
| HW JPEGエンコーダー (`hobot_jpu.ko`) 活用 | web_monitor 45-55% → 10-20% | 高（API不明） | 未着手 |
| detector 昼間スロットリング | detector 40% → ~10% | 低（設計検討要） | 未着手 |
