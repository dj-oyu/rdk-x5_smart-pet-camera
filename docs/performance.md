...
| SHM→Goヒープ転送 | `ReadLatestCopy()` + CGo `hb_mem_import` + memcpy | 小（後述） |
| NAL処理・RTPパケット化 | pion/webrtc v4 (Go) | 小 |
| SRTP暗号化 | pion/srtp v3（ソフトウェアAES） | **支配的**（後述） |
...
SRTP 暗号化（AES-128-CTR + HMAC-SHA1）が依然として全体の ~55% を占める。
このSoCは ARMv8 暗号拡張命令を搭載しないため `encryptBlockGeneric`（ソフトウェア実装）が使われる。
パケット数削減が唯一の緩和策だが、Tailscale MTU=1280 で RTP MTU=1200 は既にほぼ上限のため拡大余地なし。

### OP-TEE ハードウェア暗号エンジン調査（2026-04-16）

RDK X5 は OP-TEE (ARM TrustZone) ベースのハードウェア暗号エンジンを搭載。
カーネル Crypto API に priority=400（generic=100 より優先）で44個の `-te` ドライバが登録済み。

**SRTP に必要なドライバの存在確認**:

| アルゴリズム | ドライバ名 | priority | AF_ALG アクセス |
|---|---|---|---|
| `ctr(aes)` | `ctr-aes-te` | 400 | 可能 |
| `hmac(sha1)` | `hmac-sha1-te` | 400 | 可能 |

**AF_ALG ベンチマーク**（1200 byte パケット、Python 経由）:

| 操作 | AF_ALG (TE) | Go ソフトウェア (pprof推定) | 比率 |
|------|-------------|---------------------------|------|
| AES-128-CTR | 13,355 ops/sec (75 µs) | — | — |
| HMAC-SHA1 | 4,610 ops/sec (217 µs) | — | — |
| **合計/パケット** | **~292 µs** | **~2,259 µs** | **7.7x 高速** |

180 pkt/s × 292 µs = 52.5 ms/s ≈ **CPU ~5%**（現在 ~40% → **-35pt** の削減見込み）。

**統合の課題**:
- pion/srtp は Go の `crypto/aes` + `crypto/cipher` を直接使用しており、AF_ALG を経由しない
- Go → AF_ALG 統合には:
  1. `cipher.Block` / `hash.Hash` インターフェースを AF_ALG syscall で実装
  2. pion/srtp をフォークして暗号プリミティブを差し替え
- 開発・保守コストが高い
- AF_ALG はソケット操作のためパケットごとに syscall が2回（send + recv）発生。ただしカーネル内で TE ドライバが処理するため CPU 時間は最小限

**結論**: HWオフロードの効果は大きい（~35pt CPU削減）が、pion/srtp のフォーク＋カスタム暗号実装が必要で投資対効果の判断が必要。クライアント数増加時のスケーラビリティ改善にも効く（SRTP はクライアント数に比例するため）。
...
| 施策 | 予想効果 | 難易度 |
|------|---------|-------|
| SRTP を AF_ALG (OP-TEE) へオフロード | streaming-server ~52% → ~15%（2クライアント） | 高（pion/srtp フォーク + cipher.Block 実装） |
