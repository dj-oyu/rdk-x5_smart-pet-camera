...
Go ComicCapture                     pet-album (Rust)
  ↓ comic_..._chatora.jpg            ↓
  recordings/comics/                 data/photos/
  ↓ inotifywait                      ↓ fsnotify
  sync-comics.sh                     watcher.rs
  ↓ rsync --remove-source-files      ↓ parse filename → DB insert
  転送成功 → SD側削除                 ↓ VLM queue → is_valid/caption/behavior
                                     ↓ album UI に反映
```
