...
YOLO デーモンは **検出が1件以上ある場合のみ** 共有メモリに結果を書き込む。

| 状況 | 共有メモリ書き込み | version 更新 |
|------|-------------------|-------------|
| 検出あり (1件以上) | する | する |
| 検出ゼロ | しない | しない |

**理由**:
- 検出ゼロの場合、Consumer側は「前回の検出結果をそのまま使う」か「表示をクリア」すればよい
- 書き込み頻度を減らしてCPU負荷を軽減
- version が更新されない = 新しい検出がない、と解釈可能

**影響**:
- Consumer (DetectionBroadcaster等) は起動時に version > 0 を待つが、検出ゼロが続くと永遠に待つ
- これは正常動作であり、WARNログはあくまで「まだ初回検出がない」状態を示す
