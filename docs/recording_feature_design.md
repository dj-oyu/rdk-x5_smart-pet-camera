...
| `/api/recording/stop` | POST | 録画停止 |
| `/api/recording/status` | GET | 録画状態 |
| `/api/recording/heartbeat` | POST | ハートビート（自動停止防止） |
| `/api/recordings` | GET | 録画一覧 |
| `/api/recordings/{name}` | GET | 録画/サムネイルダウンロード |
| `/api/recordings/{name}` | DELETE | 録画削除（サムネイルも削除） |
| `/api/recordings/{name}/thumbnail` | POST | サムネイル再生成 |
...
```json
{
  ...
  "started_at": 1738728896
}
```

**エラー:**
- `{"error": "already recording"}` - 既に録画中
- `{"error": "conversion in progress"}` - 変換処理中
...
```json
{
  ...
  "stopped_at": 1738728911
}
```

#### `GET /api/recording/status` - 録画状態
...
| `converting` | bool | MP4変換＋サムネイル生成中かどうか |
| `filename` | string | 最後の録画ファイル名 |
| `frame_count` | int | 録画フレーム数 |
| `bytes_written` | int | 書き込みバイト数 |
| `duration_ms` | int | 録画時間（ミリ秒） |
| `stop_reason` | string | 自動停止の理由（`"heartbeat timeout"`, `"max duration reached"`, 手動停止なら空） |

#### `POST /api/recording/heartbeat` - ハートビート

録画中に定期的に呼び出すことで自動停止を防止（タイムアウト: 3秒）。
...
```json
{
  "recordings": [
    {
      "name": "recording_20260205_123456.mp4",
      "size_bytes": 47185920,
      "created_at": "2026-02-05T12:34:56+09:00",
      "thumbnail": "recording_20260205_123456.jpg"
    },
    {
      "name": "recording_20260205_120000.mp4",
      "size_bytes": 23456789,
      "created_at": "2026-02-05T12:00:00+09:00",
      "thumbnail": "recording_20260205_120000.jpg"
    }
  ]
}
```
...
| フィールド | 型 | 説明 |
|-----------|-----|------|
| `name` | string | ファイル名（`.mp4` または `.h264`） |
| `size_bytes` | int | ファイルサイズ |
| `created_at` | string | 作成日時（ISO 8601） |
| `thumbnail` | string | サムネイルファイル名（存在する場合のみ） |

**備考:** サムネイルがない既存録画は、一覧取得時にバックグラウンドで自動生成されます。

#### `GET /api/recordings/{name}` - ファイルダウンロード

- `.mp4` / `.h264`: `Content-Disposition: attachment` でダウンロード
- `.jpg`: `Content-Type: image/jpeg` でインライン表示（サムネイル）

#### `DELETE /api/recordings/{name}` - 録画削除

録画ファイルと対応するサムネイル（`.jpg`）を同時に削除。

**レスポンス:**
```json
{"deleted": true, "filename": "recording_20260205_123456.mp4"}
```

#### `POST /api/recordings/{name}/thumbnail` - サムネイル再生成

任意のタイムスタンプでサムネイルを再生成。
...
{
  "success": true,
  "filename": "recording_20260205_123456.mp4",
  "timestamp": 5.5
}
```
...
**使用例:**
```bash
curl -X POST http://localhost:8080/api/recordings/recording_20260205_123456.mp4/thumbnail \
...
- [x] サーバー側Recording Service（Go） ← `internal/webmonitor/h264_recorder.go`
- [x] H.264 NAL直接保存（I/Oのみ、CPU負荷ゼロ）
- [x] ffmpeg mux処理（録画停止後）
- [x] ダウンロードAPI実装
- [x] フロントエンドをサーバーAPI呼び出しに統一 ← `monitor.js` RecordingManager書き換え
- [x] 変換中の状態管理（`converting`フラグ）
- [x] 変換中のボタン無効化・グレーアウトUI

### Phase 2.5: サムネイル機能
- [x] MP4変換完了後にサムネイル自動生成（3秒目 or 最初のフレーム）
- [x] 録画一覧APIに`thumbnail`フィールド追加
- [x] サムネイル配信（`/api/recordings/{name}.jpg`）
- [x] 既存録画のサムネイル自動生成（一覧取得時にバックグラウンド実行）
- [x] 録画削除時にサムネイルも同時削除
- [x] YOLO検出時刻でのサムネイル生成（最初の検出フレームを使用）
- [x] サムネイル再生成API（`POST /api/recordings/{name}/thumbnail`）
- [x] 録画一覧UIでサムネイル表示
- [x] サムネイルクリックで拡大プレビューモーダル
...
- [x] `/api/recordings` 一覧API
- [x] `/api/recordings/{name}` ダウンロード/削除API
- [ ] M5Stack側サンプルコード
- [ ] タッチUI実装

### Phase 5: 改善
- [ ] ストレージ管理（古い録画の自動削除）
- [x] 録画一覧UI（Web）← モーダルで実装済み
- [ ] 録画中のLED/通知インジケーター
