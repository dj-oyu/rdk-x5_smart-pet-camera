...
| マシン | デバイス名 | FQDN | 役割 |
|--------|-----------|------|------|
| RDK X5 | `rdk-x5` | `rdk-x5.tail848eb5.ts.net` | ペットカメラ (映像配信・YOLO検出) |
| M5Stack AI Pyramid | `m5stack-ai-pyramid` | `m5stack-ai-pyramid.tail848eb5.ts.net` | VLM行動解析サーバー |
...
Tailscale 証明書 (`rdk-x5.tail848eb5.ts.net.crt`) を使用して Go Web Monitor (:8080) を HTTPS 化している。
