...
| M5Stack AI Pyramid | `<album-host>` | `<album-host>` | VLM行動解析サーバー |

### 1.3 システム構成

```mermaid
graph LR
    subgraph RDK["RDK X5<br/><camera-host>"]
        CAM["Camera → YOLO"]
    ...
    subgraph PYRAMID["M5Stack AI Pyramid<br/><album-host>"]
...
ブラウザから見える通信は全て `https://<camera-host>:8080` に集約される。
バックエンドへの通信（:8081, <album-host>:8090）はサーバー間通信として Go がProxyする。
...
Browser (HTTPS)
  └── https://<camera-host>:8080
        ...
        └── /api/vlm/*               → Proxy → <album-host>:8090 (新規)
```
...
```mermaid
graph TD
    MJPEG["camera<br/>GET /stream (MJPEG)"] -->|"Frame capture"| FRAME["Frame Acquisition<br/>(Periodic capture)"]
    SSE["camera<br/>GET /api/detections/stream (SSE)"] -->|"Detection events"| EVENT["YOLO Detection Event Receiver"]
    ...
    BROWSER["Browser<br/>https://<camera-host>:8080/api/vlm/*"] -->|Proxy| GOPROXY["camera Go Server"]
    GOPROXY -->|Forward| LOGAPI["http://<album-host>:8090/api/*"]
    LOGDB --> LOGAPI
```
...
### 3.1 rdk-x5 → <album-host> （既存APIの利用）
...
```
GET http://<camera-host>:8080/stream
...
```
GET http://<camera-host>:8080/api/detections/stream
...
### 3.2 <album-host> 行動ログAPI（新規）

行動ログを配信するREST APIを <album-host> 側に実装する。
ブラウザ（Monitor UI）から Tailscale 経由で直接アクセスする。

**ベースURL**: `http://<album-host>:8090`

**CORS設定**: `Access-Control-Allow-Origin: http://<camera-host>:8080`

#### 3.2.1 行動ログ一覧取得

```
GET http://<album-host>:8090/api/behavior-logs
```
...
```
GET http://<album-host>:8090/api/behavior-logs/{log_id}
```
...
```
GET http://<album-host>:8090/api/behavior-logs/{log_id}/thumbnail
...
```
GET http://<album-host>:8090/api/behavior-logs/stream
Content-Type: text/event-stream
```
...
```
GET http://<album-host>:8090/api/behavior-summary
```
...
```
GET http://<album-host>:8090/api/behavior-trends
```
...
```
GET http://<album-host>:8090/health
```
...
camera:
  host: "<camera-host>"  # Tailscale MagicDNS名
  mjpeg_port: 8080
  sse_port: 8080
...
2. **Tailscale共有**: camera を別アカウントに share している。
   albumを追加で share するのは管理が煩雑
3. **mixed-content回避**: HTTPS ページから HTTP API への直接アクセスはブラウザがブロックする

Proxyにより、ブラウザからは全て `https://<camera-host>:8080` への通信で完結する。

#### Proxyエンドポイント

| フロントエンドURL | Proxy先 |
|------------------|---------|
| `GET /api/vlm/behavior-logs` | `http://<album-host>:8090/api/behavior-logs` |
| `GET /api/vlm/behavior-logs/{id}` | `http://<album-host>:8090/api/behavior-logs/{id}` |
| `GET /api/vlm/behavior-logs/{id}/thumbnail` | `http://<album-host>:8090/api/behavior-logs/{id}/thumbnail` |
| `GET /api/vlm/behavior-logs/stream` | `http://<album-host>:8090/api/behavior-logs/stream` (SSE) |
| `GET /api/vlm/behavior-summary` | `http://<album-host>:8090/api/behavior-summary` |
| `GET /api/vlm/behavior-trends` | `http://<album-host>:8090/api/behavior-trends` |
| `GET /api/vlm/health` | `http://<album-host>:8090/health` |
...
type Config struct {
    // ... 既存フィールド
    VLMBaseURL string  // "http://<album-host>:8090"
}
...
```yaml
# 起動引数（またはスクリプト内で指定）
VLM_BASE_URL="http://<album-host>:8090"
```
...
| albumに接続不可 | — | Go Proxyが502を返す → 行動ログ欄に「VLMサーバー接続なし」表示 |
...
- [ ] Go Web Monitor に VLM Proxy 追加 (`/api/vlm/*` → <album-host>:8090)
...
```mermaid
graph TD
    subgraph TS["Tailscale Network (<tailscale-tenant>)<br/>WireGuard Encrypted Tunnel"]
        subgraph RDK["<camera-host>"]
        ...
        subgraph M5["<album-host>"]
            MJPEG_IN["MJPEG Capture (HTTP)"]
            SSE_IN["SSE Detection Receiver (HTTP)"]
            LOGAPI[":8090 Behavior Log API (HTTP)"]
        end

        BROWSER["Browser (PC / iPhone Safari)<br/>https://<camera-host>:8080/*<br/>/stream (MJPEG)<br/>/api/webrtc/offer (Proxy)<br/>/api/vlm/* (Proxy)"]
    end
...
> **Note**: Only rdk-x5 is shared externally. The browser only needs to know rdk-x5. <album-host> does not need to be shared (rdk-x5 acts as proxy).
