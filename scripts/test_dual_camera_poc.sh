#!/bin/bash
# Test dual camera PoC with Low-level API

echo "=== Starting Dual Camera PoC Test ==="
...
SHM_NAME_H264=/pet_camera_stream_day /app/smart-pet-camera/build/camera_poc_lowlevel -C 0 > /tmp/poc_cam0.log 2>&1 &
CAM0_PID=$!
...
SHM_NAME_H264=/pet_camera_stream_night /app/smart-pet-camera/build/camera_poc_lowlevel -C 1 > /tmp/poc_cam1.log 2>&1 &
CAM1_PID=$!
...
tail -30 /tmp/poc_cam0.log

echo ""
echo "=== Camera 1 Log (last 30 lines) ==="
tail -30 /tmp/poc_cam1.log
...
tail -10 /tmp/poc_cam0.log

echo ""
echo "=== Camera 1 Final Log (last 10 lines) ==="
tail -10 /tmp/poc_cam1.log
