...
```mermaid
graph TD
    sensor["D-Robotics Camera<br/>- IMX219 Sensor<br/>- MIPI Interface"]
    hw["VIN → ISP → VSE<br/>(D-Robotics Hardware)<br/>RAW10 → NV12 → Scale"]
    daemon["Camera Daemon (C)<br/>- JPEG Encoding<br/>- 30fps"]
    ...
    sensor -->|"Hardware Pipeline"| hw
    hw -->|"NV12 → JPEG (libjpeg)"| daemon
    daemon -->|"Write (atomic)"| shm
...
現在はゼロコピー設計のため、旧リングバッファのアライメント問題は解消済みです。
`real_shared_memory.py` の `CZeroCopyFrame` / `CZeroCopyFrameBuffer` が現在の構造体定義です。
