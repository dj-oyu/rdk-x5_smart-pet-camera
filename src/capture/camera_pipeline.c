...
int pipeline_create(camera_pipeline_t* pipeline, int camera_index, int sensor_width,
    ...
    LOG_INFO(Pipeline_log_header, "Creating pipeline for Camera %d (%dx%d@%dfps, %dkbps)",
    ...
    // Create VIO context
    ret = vio_create(&pipeline->vio, camera_index, sensor_width, sensor_height, output_width,
                     output_height, fps);
    if (ret != 0) {
    ...
    ret =
        encoder_create(&pipeline->encoder, camera_index, output_width, output_height, fps, bitrate);
    if (ret != 0) {
    ...
    if (!pipeline->shm_h265_zc) {
        LOG_ERROR(Pipeline_log_header, "Failed to create H.265 zero-copy SHM: %s",
                  SHM_NAME_H265_ZC);
        ret = -1;
        goto error_cleanup;
    }
    ...
    if (!pipeline->shm_yolo_zerocopy) {
        LOG_ERROR(Pipeline_log_header, "Failed to create zero-copy shared memory: %s",
                  SHM_NAME_YOLO_ZC);
        ret = -1;
        goto error_cleanup;
    }
    LOG_INFO(Pipeline_log_header, "Zero-copy shared memory created: %s", SHM_NAME_YOLO_ZC);

    // MJPEG input NV12 (768x432 from VSE Channel 2, always written when active, writable by
    // web_monitor)
    pipeline->shm_mjpeg_zc = shm_zerocopy_create(SHM_NAME_MJPEG_ZC);
    if (!pipeline->shm_mjpeg_zc) {
        LOG_ERROR(Pipeline_log_header, "Failed to open/create MJPEG frame shared memory: %s",
                  SHM_NAME_MJPEG_ZC);
        ret = -1;
        goto error_cleanup;
    }
    ...
    if (pipeline->camera_index == 1) {
        ...
        for (int i = 0; i < NUM_ROI_REGIONS; i++) {
            ...
            if (!pipeline->shm_roi_zc[i]) {
                LOG_ERROR(Pipeline_log_header, "Failed to create ROI SHM[%d]: %s", i, roi_names[i]);
                ret = -1;
                goto error_cleanup;
            }
        }
    ...
    ret =
        encoder_thread_create(&pipeline->encoder_thread, &pipeline->encoder, pipeline->shm_h265_zc,
                              output_width, output_height, pipeline->vio.vse_handle);
    if (ret != 0) {
...
int pipeline_run(camera_pipeline_t* pipeline, volatile bool* running_flag) {
    ...
    LOG_INFO(Pipeline_log_header, "Starting capture loop (threaded encoder, 30fps NV12+H.264)...");
    ...
    while (*running_flag) {
        ...
        bool write_active =
            pipeline->active_camera && *pipeline->active_camera == pipeline->camera_index;
        if (!write_active) {
        ...
        if (ret != 0) {
            ...
            if (is_active && ret != -43) {
                ...
                LOG_DEBUG(Pipeline_log_header, "vio_get_frame failed: %d (active=%d)", ret,
                          is_active);
            }
            continue;
        }
        ...
        if (frame_number == 1 && pipeline->camera_index == 1) {
            ...
            if (nr_ret != 0) {
                LOG_WARN(Pipeline_log_header, "Night 3DNR setup failed: %d (retrying at frame 30)",
                         nr_ret);
            } else {
...
#define ISP_BRIGHTNESS_MASK_NIGHT        127 // 128 frames (~4.3 sec, 2^7 - 1)
        ...
        if (is_day_camera) {
            brightness_mask =
                write_active ? ISP_BRIGHTNESS_MASK_DAY_ACTIVE : ISP_BRIGHTNESS_MASK_DAY_INACTIVE;
        } else {
        ...
        if (frame_number % 30 == 0) {
            LOG_DEBUG(Pipeline_log_header, "Flags: is_active=%d, brightness=%.1f lux=%u zone=%d",
                      write_active, brightness_result.brightness_avg,
        ...
        if (write_active) {
            ret = encoder_thread_push_frame(&pipeline->encoder_thread, &vio_frame, frame_number,
                                            pipeline->camera_index, frame_timestamp);

            if (ret == 0) {
                frame_owned_by_encoder = true;
            } else {
                LOG_WARN(Pipeline_log_header, "Encoder queue full, frame %d dropped", frame_number);
            }
        }
                ...
                if (zc_ret == 0) {
                    ...
                    if (frame_number == 0) {
                        ...
                        LOG_DEBUG(Pipeline_log_header,
                                  ...
                                  sizeof(hb_mem_graphic_buf_t), gb->width, gb->height, gb->format,
                                  gb->plane_cnt, gb->stride, gb->vstride, gb->is_contig,
                                  (long)gb->flags, gb->fd[0], gb->fd[1], gb->fd[2], gb->share_id[0],
                                  gb->share_id[1], gb->share_id[2], (unsigned long)gb->size[0],
                                  (unsigned long)gb->size[1], (unsigned long)gb->size[2],
                                  (unsigned long)gb->offset[0], (unsigned long)gb->offset[1],
                                  (unsigned long)gb->offset[2], (unsigned long)gb->phys_addr[0],
                        ...
                        LOG_DEBUG(
                            ...
                            r[101], r[102], r[103], r[104], r[105], r[106], r[107], r[108], r[109],
                            r[110], r[111], r[112], r[113], r[114], r[115], r[116], r[117], r[118],
                            r[119], r[120], r[121], r[122], r[123], r[124], r[125], r[126], r[127],
                            r[128], r[129], r[130], r[131], r[132], r[133], r[134], r[135], r[136],
                            r[137], r[138], r[139], r[140], r[141], r[142], r[143], r[144], r[145],
        ...
        if (write_active && pipeline->camera_index == 1) {
            for (int i = 0; i < NUM_ROI_REGIONS; i++) {
                ...
                if (roi_ret == 0) {
                    if (frame_number == 0) {
                        LOG_DEBUG(Pipeline_log_header,
                                  "ROI[%d]: %dx%d, planes=%d, share_id=[%d,%d]", i,
                                  roi_frame.buffer.width, roi_frame.buffer.height,
                    ...
                    for (int p = 0; p < roi_frame.buffer.plane_cnt && p < ZEROCOPY_MAX_PLANES;
                         p++) {
        ...
        if (write_active) {
            ...
            if (ret == 0) {
                ...
                memcpy(mjpeg_zc.hb_mem_buf_data, &mjpeg_frame.buffer, sizeof(mjpeg_frame.buffer));

                int write_ret = shm_zerocopy_write(pipeline->shm_mjpeg_zc, &mjpeg_zc);
                if (write_ret == 0) {
                    if (frame_number == 0) {
                        LOG_INFO(Pipeline_log_header,
                                 "VSE Ch2 output: %dx%d (zero-copy, share_id=%d)", mjpeg_zc.width,
                                 mjpeg_zc.height, mjpeg_zc.share_id[0]);
                    }
                    // Consumer finished with previous frame - safe to release it now
        ...
        if (!night_awb_applied && frame_number == 30) {
            ...
            if (awb_ret != 0) {
                ...
                LOG_INFO(Pipeline_log_header,
                         "Night camera AWB fixed: R=1.8 G=1.8 B=2.34 (frame %d)", frame_number);
            }
        ...
        if (frame_number % 30 == 0) {
            ...
            LOG_DEBUG(Pipeline_log_header, "Frame %d, FPS: %.2f, H.264 encoded: %lu, dropped: %lu",
    ...
    LOG_INFO(Pipeline_log_header, "Completed: %d frames in %.2f seconds (avg FPS: %.2f)",
             frame_number, total_elapsed, avg_fps);
    LOG_INFO(Pipeline_log_header, "H.264 encoded: %lu, dropped: %lu",
             pipeline->encoder_thread.frames_encoded, pipeline->encoder_thread.frames_dropped);
