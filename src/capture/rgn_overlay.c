...
static void draw_rect(uint8_t* nv12, int w, int h, const overlay_rect_t* r) {
...
static void draw_text(uint8_t* nv12, int w, int h, const overlay_text_t* t) {
    ...
    for (int ci = 0; ci < len; ci++) {
        ...
        if (fi < 0 || fi >= (int)(sizeof(font5x7) / sizeof(font5x7[0])))
            fi = 14;
...
int rgn_overlay_draw(uint8_t* nv12, int width, int height, const overlay_rect_t* rects,
                     int num_rects, const overlay_text_t* texts, int num_texts) {
