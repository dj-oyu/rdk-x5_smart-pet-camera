...
static void* encoder_thread_worker(void* arg) {
    ...
    while (ctx->running) {
        ...
        int ret = encoder_encode_frame_zerocopy(
            ctx->encoder, (const uint8_t*)frame->vse_frame.buffer.virt_addr[0],
            (const uint8_t*)frame->vse_frame.buffer.virt_addr[1], frame->vse_frame.buffer.size[0],
            frame->vse_frame.buffer.size[1], 2000, &enc_out);
        ...
        if (ret == 0 && enc_out.data_size > 0) {
            ...
            const uint64_t encoded = __atomic_add_fetch(&ctx->frames_encoded, 1, __ATOMIC_RELAXED);
            if (encoded % 300 == 0) {
                const uint64_t dropped = __atomic_load_n(&ctx->frames_dropped, __ATOMIC_RELAXED);
                LOG_INFO("EncoderThread", "Encoder stats: encoded=%lu, dropped=%lu (%.1f%%)",
                         encoded, dropped, (float)dropped / (encoded + dropped) * 100.0f);
            }
        } else {
            if (ret != 0) {
    ...
    LOG_INFO("EncoderThread", "Worker stopped (encoded=%lu, dropped=%lu)", ctx->frames_encoded,
             ctx->frames_dropped);
    return NULL;
}
...
int encoder_thread_create(encoder_thread_t* ctx, encoder_context_t* encoder,
    ...
    LOG_INFO("EncoderThread", "Created (queue_size=%d, full zero-copy mode)", ENCODER_QUEUE_SIZE);
    return 0;
}
...
int encoder_thread_push_frame(encoder_thread_t* ctx, const hbn_vnode_image_t* vse_frame,
    ...
    const uint32_t write_idx = __atomic_load_n(&ctx->write_index, __ATOMIC_ACQUIRE);
    const uint32_t read_idx = __atomic_load_n(&ctx->read_index, __ATOMIC_ACQUIRE);
    ...
    const uint32_t slot = write_idx % ENCODER_QUEUE_SIZE;
