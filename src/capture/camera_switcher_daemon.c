...
static int spawn_daemon(CameraMode camera) {
    ...
    if (pid == 0) {
        ...
        execl(CAPTURE_BIN, CAPTURE_BIN, "-C", camera_arg, "-W", "1280", "-H", "720", NULL);
        perror("execl");
        _exit(1);
    }
...
static ZeroCopyFrameBuffer* wait_for_zerocopy_shm(const char* name, int max_retries) {
...
static void do_switch(SwitcherContext* ctx, CameraMode target, const char* reason) {
    ...
    LOG_INFO("SwitcherDaemon", "Switch: %s (%s)", target == CAMERA_MODE_DAY ? "DAY" : "NIGHT",
             reason);
}
...
static int switcher_loop(SwitcherContext* ctx) {
    ...
    while (!g_stop) {
        ...
        CameraSwitchDecision decision =
            camera_switcher_record_brightness(&ctx->switcher, CAMERA_MODE_DAY, (double)brightness);
        ...
        int interval_ms =
            (ctx->active_camera == CAMERA_MODE_DAY) ? POLL_INTERVAL_DAY_MS : POLL_INTERVAL_NIGHT_MS;
