...
static char doc[] = "camera_daemon_drobotics -- D-Robotics camera daemon with shared memory";
static char args_doc[] = "";
static struct argp_option options[] = {
    {"preset", 'P', "N", 0, "Resolution/FPS preset: 1=640x480@30, 2=1920x1080@30"},
    {"width", 'w', "PIXELS", 0, "YUV output width (default: 640)"},
    {"height", 'h', "PIXELS", 0, "YUV output height (default: 480)"},
    {"sensor-width", OPT_SENSOR_WIDTH, "PIXELS", 0, "Sensor raw width (default: 0 = auto)"},
    {"sensor-height", OPT_SENSOR_HEIGHT, "PIXELS", 0, "Sensor raw height (default: 0 = auto)"},
    ...
    {"count", 'c', "N", 0, "Number of frames to capture (default: 0 = infinite)"},
    {0}
};
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    case 'w': args->out_width = atoi(arg); break;
    case 'h': args->out_height = atoi(arg); break;
    case OPT_SENSOR_WIDTH: args->sensor_width = atoi(arg); break;
    case OPT_SENSOR_HEIGHT: args->sensor_height = atoi(arg); break;
    case 'f': args->fps = atoi(arg); break;
    case 'C': args->camera_index = atoi(arg); break;
    case OPT_DAEMON: args->daemon_mode = 1; break;
...
int main(int argc, char **argv) {
    ...
    if (args.sensor_width <= 0) args.sensor_width = SENSOR_WIDTH_DEFAULT;
    if (args.sensor_height <= 0) args.sensor_height = SENSOR_HEIGHT_DEFAULT;
    ...
    while (g_running && ((frame_limit == 0) || (frame_counter < frame_limit))) {
        ...
        if (encode_nv12_to_jpeg(y_plane, uv_plane, ctx.out_width, ctx.out_height,
                                 y_stride, uv_stride,
                                 shm_frame.data, &shm_frame.data_size,
                                 MAX_FRAME_SIZE) < 0) {
        ...
        if (frame_counter % 30 == 0) {
            printf("[Info] Frame %lu captured (%zu bytes)\n",
                   frame_counter, shm_frame.data_size);
        }
    }
