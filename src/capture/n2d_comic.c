...
static int ensure_canvas(int w, int h) {
    if (g_canvas.memory && g_canvas_w == w && g_canvas_h == h)
        return 0;
    if (g_canvas.memory)
        n2d_free(&g_canvas);
    memset(&g_canvas, 0, sizeof(g_canvas));
    n2d_error_t err =
        n2d_util_allocate_buffer(w, h, N2D_NV12, N2D_0, N2D_LINEAR, N2D_TSC_DISABLE, &g_canvas);
    if (N2D_IS_ERROR(err)) {
...
static int ensure_src(int w, int h) {
    if (g_src_buf.memory && g_src_w == w && g_src_h == h)
        return 0;
    if (g_src_buf.memory)
        n2d_free(&g_src_buf);
    memset(&g_src_buf, 0, sizeof(g_src_buf));
    n2d_error_t err =
        n2d_util_allocate_buffer(w, h, N2D_NV12, N2D_0, N2D_LINEAR, N2D_TSC_DISABLE, &g_src_buf);
    if (N2D_IS_ERROR(err)) {
...
int n2d_comic_compose(const uint8_t* nv12_frames[], const int frame_widths[],
                      ...
                      int out_h) {
    if (!nv12_frames || num_panels < 1 || num_panels > 4)
        return -1;
    if (!out_nv12)
        return -1;

    if (ensure_n2d_init() != 0)
        return -1;
    if (ensure_canvas(out_w, out_h) != 0)
        return -1;
    ...
    for (int i = 0; i < num_panels && i < 4; i++) {
        ...
        if (crops && crops[i].src_w > 0 && crops[i].src_h > 0) {
            src_rect =
                (n2d_rectangle_t){crops[i].src_x, crops[i].src_y, crops[i].src_w, crops[i].src_h};
        } else {
