...
func (m *Monitor) Snapshot() (MonitorStats, SharedMemoryStats, *DetectionResult, []DetectionResult) {
	...
	shmStats := SharedMemoryStats{
		...
		DetectionVersion:   m.detectionVersion,
...
// LatestJPEG returns the most recent JPEG frame from shared memory.
func (m *Monitor) LatestJPEG() ([]byte, bool) {
	if m.shm == nil {
		return nil, false
	}
	return m.shm.LatestJPEG()
}
